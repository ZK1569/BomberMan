...
{
  for (int i = 0; i < 3; ++i)
  { // TODO: changer le 3 en nombre de jouer
...
void kickBomb(Map *map, char direction, Player *player, Config *config)
{
  ...
  case MOVE_UP:
    ...
    {
      if (player->y - i >= 1 && map->map[player->y - i][player->x] == '0')
        continue;
  ...
  case MOVE_RIGHT:
    for (int i = 2; i <= kickRange; ++i)
    {
      if (player->x + i <= config->rows - 1 && map->map[player->y][player->x + i] == '0')
        continue;
  ...
  case MOVE_DOWN:
    for (int i = 2; i <= kickRange; ++i)
    {
      if (player->y + i < config->columns - 1 && map->map[player->y + i][player->x] == '0')
        continue;
      map->map[player->y + 1][player->x] = '0';
  ...
  case MOVE_LEFT:
    for (int i = 2; i <= kickRange; ++i)
    {
      if (player->x - i >= 0 && map->map[player->y][player->x - i] == '0')
        continue;
      map->map[player->y][player->x - 1] = '0';
...
int move(char direction, Player *player, Map *map, Player *allPlayers, Config *config, int nbrPlayers)
{
  ...
  if (player->nbrBomb >= 1)
  { // If the player has at less a bomb
    for (int i = 0; i < 3; ++i)
    { // 3 = nbr bomb (a mettre dans une variable)
      ...
      if (!(player->myBomb[i].x == 0 && player->myBomb[i].y == 0))
      {                           // Pour les bombes qu'il a mais qui sont sur la map et pas en reserve
